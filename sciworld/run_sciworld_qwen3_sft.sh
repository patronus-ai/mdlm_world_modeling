...
RUN_NAME="sciworld_qwen3_sft_$(date +%Y%m%d_%H%M%S)"
...
swift sft \
    --model $MODEL_PATH \
    --template qwen3 \
    --dataset /workspace/user/sciworld_training/data/sciworld_sft_gpt_perfect_full_prompt.jsonl \
    ...
    --report_to wandb \
    --run_name "$RUN_NAME" \
    --output_dir output/sciworld_qwen3_sft

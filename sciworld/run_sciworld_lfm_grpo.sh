#!/bin/bash
# GRPO — LFM2.5-1.2B on ScienceWorld tasks with WM (v2 fixes)
# Smaller model: higher LR, more vllm mem, shorter context
...
RUN_NAME="sciworld_lfm25_sft_grpo_$(date +%Y%m%d_%H%M%S)"
...
swift rlhf \
    ...
    --dataset /workspace/user/sciworld_training/data/sciworld_rl_split.jsonl \
    ...
    --report_to wandb \
    --run_name "$RUN_NAME" \
    --output_dir output/sciworld_lfm25_sft_grpo

#!/bin/bash
# GRPO — Mistral-7B-Instruct-v0.3 on AppWorld RL tasks with WM
...
export TRAJECTORY_LOG=/tmp/appworld_mistral_trajectories.jsonl
...
RUN_NAME="appworld_mistral_grpo_$(date +%Y%m%d_%H%M%S)"
...
swift rlhf \
    ...
    --report_to wandb \
    --run_name "$RUN_NAME" \
    --output_dir output/appworld_mistral_grpo
